...
type UserSummary = { id: string; username?: string; name?: string; is_superuser?: boolean; is_org_admin?: boolean };
...
function getJumpServerServiceHeaders() { return { Authorization: `Bearer ${JUMPSERVER_SERVICE_TOKEN}`, 'X-JMS-ORG': JUMPSERVER_ORG_ID }; }
...
function extractResults(data: any): any[] { return Array.isArray(data) ? data : Array.isArray(data?.results) ? data.results : Array.isArray(data?.data) ? data.data : Array.isArray(data?.data?.results) ? data.data.results : []; }
...
async function getAuthenticatedUser(req: Request): Promise<UserSummary> {
  ...
  return { id: String(profile.id), username: profile.username, name: profile.name, is_superuser: profile.is_superuser, is_org_admin: profile.is_org_admin };
}

async function fetchApplyAssetFlow(req: Request): Promise<any> {
  const requestFlow = async (headers: Record<string, string>) => {
    const response = await axios.get(`${JUMPSERVER_URL}/api/v1/tickets/flows/`, { headers, params: { type: 'apply_asset', limit: 200 }, timeout: 15000 });
    const flows = extractResults(response.data);
    return flows.find((item: any) => (typeof item?.type === 'string' ? item.type : item?.type?.value) === 'apply_asset') || flows[0] || null;
  };
  try {
    return await requestFlow(getJumpServerHeaders(req));
  } catch (error) {
    const axiosError = error as AxiosError;
    if ((axiosError.response?.status === 401 || axiosError.response?.status === 403) && JUMPSERVER_SERVICE_TOKEN) return await requestFlow(getJumpServerServiceHeaders());
    throw error;
  }
}

async function resolvePortalRole(req: Request): Promise<'admin' | 'approver' | 'user'> {
  const user = await getAuthenticatedUser(req);
  if (user.is_superuser === true || user.is_org_admin === true) return 'admin';
  if (!JUMPSERVER_SERVICE_TOKEN) throw new Error('JUMPSERVER_SERVICE_TOKEN is required to resolve Ticket Flow approvers for non-admin users');
  const flow = await fetchApplyAssetFlow(req);
  if (!flow) return 'user';
...
app.get('/portal-api/access', requireAuth, async (req, res) => {
  try {
    const user = await getAuthenticatedUser(req);
    const role = await resolvePortalRole(req);
    return res.json({ success: true, role, user: { id: user.id, username: user.username, name: user.name, is_superuser: user.is_superuser, is_org_admin: user.is_org_admin } });
  } catch (error) {
    const axiosError = error as AxiosError;
    if (axiosError.response) return res.status(axiosError.response.status).json({ success: false, message: 'Failed to resolve portal access from JumpServer', details: axiosError.response.data });
    console.error('Resolve portal access error:', error);
    return res.status(503).json({ success: false, message: error instanceof Error ? error.message : 'Failed to resolve portal access' });
  }
});
...
async function canCurrentUserSeeTicket(req: Request, ticket: any, currentUserId: string): Promise<boolean> {
...
async function assertApprovalAccess(req: Request, ticketId: string): Promise<{ allowed: boolean; ticket?: any; reason?: string }> {
  ...
  const allowed = await canCurrentUserSeeTicket(req, ticket, currentUser.id);
...
async function listTickets(req: Request, res: Response, state?: string) {
  try {
    if (!JUMPSERVER_URL) return res.status(500).json({ success: false, message: 'JUMPSERVER_URL is not configured' });
    const response = await axios.get(`${JUMPSERVER_URL}/api/v1/tickets/apply-asset-tickets/`, { headers: getJumpServerHeaders(req), params: state ? { state, limit: 200, ordering: '-date_created' } : { limit: 200, ordering: '-date_created' }, timeout: 15000 });
    const tickets = extractResults(response.data);
    if (state !== 'pending') return res.json({ success: true, count: tickets.length, tickets });
    const currentUser = await getAuthenticatedUser(req);
    const pendingTickets = await fetchPendingTickets(req);
    const visible = pendingTickets.filter((ticket) => canCurrentUserSeeTicket(req, ticket, currentUser.id));
