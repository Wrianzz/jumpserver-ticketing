...
async function fetchApplyAssetFlow(req: Request): Promise<any> {
  ...
  try {
    ...
    const axiosError = error as AxiosError;
    if ((axiosError.response?.status === 401 || axiosError.response?.status === 403) && JUMPSERVER_SERVICE_TOKEN) {
      return await requestFlow(getJumpServerServiceHeaders());
    }
    throw error;
  }
}
...
async function resolvePortalRole(req: Request): Promise<'admin' | 'approver' | 'user'> {
  ...
  if (!JUMPSERVER_SERVICE_TOKEN) {
    throw new Error('JUMPSERVER_SERVICE_TOKEN is required to resolve Ticket Flow approvers for non-admin users');
  }
