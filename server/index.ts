...
const TEAM_GROUPS = new Set((process.env.TEAM_GROUPS || '').split(',').map((name) => name.trim()).filter(Boolean));
...
function configuredTeamNames(names: string[]): Set<string> { if (TEAM_GROUPS.size > 0) return new Set(names.filter((name) => TEAM_GROUPS.has(name))); return new Set(names.filter((name) => !TEAM_GROUPS_IGNORE.has(name.toLowerCase()))); }
...
async function findUser(req: Request, identifier: string, cache: Map<string, UserSummary | null>): Promise<UserSummary | null> {
  ...
  const response = await axios.get(`${JUMPSERVER_URL}/api/v1/users/users/`, { headers: getJumpServerHeaders(req), params: { search: identifier.trim(), limit: 10, fields_size: 'mini' }, timeout: 15000 });
  const users = extractResults(response.data) as UserSummary[];
  const exact = users.find((user) => String(user.username || '').toLowerCase() === key) || users.find((user) => String(user.name || '').toLowerCase() === key) || users[0] || null;
  cache.set(key, exact);
  return exact;
}
...
async function canCurrentUserSeeTicket(req: Request, ticket: any, currentUserId: string, userCache: Map<string, UserSummary | null>, groupCache: Map<string, Set<string>>): Promise<boolean> {
  ...
  const [approverTeams, applicantTeams] = await Promise.all([
    getTeamGroups(req, currentUserId, groupCache),
    getTeamGroups(req, applicantUser.id, groupCache),
  ]);
...
app.listen(PORT, () => { console.log(`JumpServer Ticketing backend listening on port ${PORT}`); console.log(`JumpServer URL: ${JUMPSERVER_URL || '(NOT CONFIGURED)'}`); console.log(`JumpServer org: ${JUMPSERVER_ORG_ID}`); console.log(`Configured team groups: ${TEAM_GROUPS.size > 0 ? [...TEAM_GROUPS].join(', ') : '(all groups except ignored)'}`); });
