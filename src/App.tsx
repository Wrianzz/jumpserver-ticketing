...
import { AlertTriangle, LogOut, Filter, CheckCircle, Workflow, ShieldCheck, X, ChevronDown, ChevronRight } from 'lucide-react';
...
function AppLayout({ onLogout, userRole, user }: { onLogout: () => void; userRole: string | null; user: AuthUser | null; }) {
    ...
    <aside className="w-72 border-r border-slate-200 bg-white p-6 flex flex-col gap-8 shrink-0 hidden md:flex">
      ...
      <div className="flex flex-col gap-4"><h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Navigation</h3><nav className="flex flex-col gap-1">
        ...
        {userRole === 'admin' && <>
          ...
          <NavLink to="/uam" className={navClass}><ShieldCheck className="w-4 h-4" />User Access Matrix</NavLink>
        </>}
  ...
  </div>{confirmation && <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) closeConfirmation(); }}><div className="absolute inset-0 bg-slate-950/45 backdrop-blur-[3px]" /><div role="dialog" aria-modal="true" className="relative w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl"><div className="h-1.5 bg-[#009688]" /><div className="p-7"><div className="flex items-start gap-4"><div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full ${confirmation.destructive ? 'bg-rose-50 text-rose-600' : 'bg-[#009688]/10 text-[#009688]'}`}>{confirmation.destructive ? <AlertTriangle className="h-5 w-5" /> : <CheckCircle className="h-5 w-5" />}</div><div className="flex-1"><h3 className="text-lg font-semibold text-slate-900">{confirmation.title}</h3><p className="mt-1.5 text-sm leading-6 text-slate-500">{confirmation.message}</p></div><button type="button" onClick={closeConfirmation} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100"><X className="h-4 w-4" /></button></div><div className="mt-7 flex justify-end gap-2"><Button variant="outline" onClick={closeConfirmation}>Cancel</Button><Button onClick={confirmAction} className={confirmation.destructive ? 'bg-rose-600 hover:bg-rose-700' : 'bg-[#009688] hover:bg-[#007f73)'}>{confirmation.confirmLabel}</Button></div></div></div></div>}</div>;
}
