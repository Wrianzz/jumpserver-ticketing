  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-[#F8FAFC] font-sans antialiased">
      <Card className="w-[400px] shadow-sm border border-slate-200 bg-white rounded-2xl overflow-hidden">
        <CardHeader className="space-y-1 text-center pt-8"><div className="flex justify-center mb-4"><div className="w-12 h-12 bg-[#009688] rounded-xl flex items-center justify-center shadow-inner"><div className="w-5 h-5 border-2 border-white rotate-45"></div></div></div><CardTitle className="text-2xl font-bold tracking-tight text-slate-900">JumpServer <span className="text-[#009688]">Ticketing Portal</span></CardTitle><CardDescription className="text-slate-500 text-sm">{mfaRequired ? 'Verify your identity with your authenticator' : 'Sign in with your JumpServer credentials'}</CardDescription></CardHeader>
        {mfaRequired ? (<form onSubmit={handleMfaVerify} className="pb-4"><CardContent className="space-y-5 px-8 pt-4"><div className="rounded-lg border border-teal-100 bg-teal-50 p-4 text-center"><p className="text-sm font-semibold text-slate-800">MFA verification required</p><p className="mt-1 text-xs text-slate-500">Enter the 6-digit OTP from your authenticator app.</p></div><div className="space-y-1.5 text-left"><Label htmlFor="otp" className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Authentication Code</Label><Input id="otp" type="text" inputMode="numeric" autoComplete="one-time-code" placeholder="000000" maxLength={6} className="w-full border-slate-200 rounded-lg px-4 py-3 text-center text-lg tracking-[0.4em] font-semibold focus-visible:ring-2 focus-visible:ring-[#009688]/20 focus-visible:border-[#009688] outline-none shadow-none" value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, '').slice(0, 6))} autoFocus required /></div>{error && <div className="text-sm font-medium text-red-500 bg-red-50 p-3 rounded-md border border-red-100">{error}</div>}</CardContent><CardFooter className="px-8 pb-8 pt-4 flex-col gap-3"><button type="submit" className="w-full py-2.5 rounded-lg text-sm font-semibold bg-[#009688] text-white hover:bg-[#00796B] shadow-lg shadow-[#009688]/20 transition-all active:scale-95 disabled:opacity-70 disabled:pointer-events-none" disabled={loading || otp.length !== 6}>{loading ? 'Verifying...' : 'Verify & Sign In'}</button><button type="button" onClick={handleBackToLogin} disabled={loading} className="w-full py-2 text-sm font-medium text-slate-500 hover:text-slate-700 transition-colors disabled:opacity-50">Back to Sign In</button></CardFooter></form>) : (<form onSubmit={handleLogin} className="pb-4"><CardContent className="space-y-5 px-8 pt-4"><div className="space-y-1.5 text-left"><Label htmlFor="username" className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Username</Label><Input id="username" type="text" placeholder="Enter your username" className="w-full border-slate-200 rounded-lg px-4 py-2 text-sm focus-visible:ring-2 focus-visible:ring-[#009688]/20 focus-visible:border-[#009688] outline-none shadow-none" value={username} onChange={(e) => setUsername(e.target.value)} required /></div><div className="space-y-1.5 text-left"><Label htmlFor="password" className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Password</Label><Input id="password" type="password" placeholder="Enter your password" className="w-full border-slate-200 rounded-lg px-4 py-2 text-sm focus-visible:ring-2 focus-visible:ring-[#009688]/20 focus-visible:border-[#009688] outline-none shadow-none" value={password} onChange={(e) => setPassword(e.target.value)} required /></div>{error && <div className="text-sm font-medium text-red-500 bg-red-50 p-3 rounded-md border border-red-100">{error}</div>}</CardContent><CardFooter className="px-8 pb-8 pt-4"><button type="submit" className="w-full py-2.5 rounded-lg text-sm font-semibold bg-[#009688] text-white hover:bg-[#00796B] shadow-lg shadow-[#009688]/20 transition-all active:scale-95 disabled:opacity-70 disabled:pointer-events-none" disabled={loading}>{loading ? 'Authenticating...' : 'Sign In'}</button></CardFooter></form>)}
      </Card>
    </div>
  );
}
