...
import { Search, RotateCw, Plus, ChevronLeft, ChevronRight, X } from 'lucide-react';
...
export function History() {
  ...
  const loadTickets = async () => {
    ...
    setError('');
    try {
      const response = await apiClient.get('/portal-api/tickets');
      const data = response.data;
      if (!data?.success) throw new Error(data?.message || 'Failed to load request history');
  ...
  const visibleTickets = useMemo(() => {
    ...
    const activeTickets = tickets.filter((ticket) => ticket.state?.value !== 'closed');
    const stateFiltered = selectedStates.length === 0
      ? activeTickets
      : activeTickets.filter((ticket) => selectedStates.includes(ticket.state?.value || ''));
    if (!keyword) return stateFiltered;
    return stateFiltered.filter((ticket) =>
      [ticket.title, ticket.serial_num, ticket.applicant]
        .filter(Boolean)
        .some((value) => String(value).toLowerCase().includes(keyword))
    );
  }, [tickets, searchTerm, selectedStates]);
  ...
  useEffect(() => { setCurrentPage(1); }, [searchTerm, selectedStates]);
  ...
  const cancelTicket = async (ticket: Ticket) => {
    ...
    if (!sessionStorage.getItem('jumpserver_token')) return;
    setCancellingId(ticket.id);
    setError('');
    try {
      const response = await apiClient.put(`/portal-api/tickets/${ticket.id}/close`);
      const data = response.data;
      if (!data?.success) throw new Error(data?.message || 'Failed to cancel ticket');
      await loadTickets();
    } catch (err) {
