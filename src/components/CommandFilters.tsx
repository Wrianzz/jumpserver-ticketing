...
const parseAccounts = (accounts: string[] = []): { mode: AccountMode; names: string[] } => {
  ...
  if (accounts.includes('@SPEC')) return { mode: 'specific', names: accounts.filter((item) => item !== '@SPEC' && !item.startsWith('@') && !item.startsWith('!')) };
  ...
  if (accounts.length) return { mode: 'specific', names: accounts.filter((item) => !item.startsWith('@')) };
...
const buildAccounts = (mode: AccountMode, values: string[]) => {
  const names = values.map((item) => item.trim()).filter(Boolean);
...
const USER_ATTRIBUTE_OPTIONS: AsyncSelectOption[] = [
  { value: 'name', label: 'Name' },
  { value: 'username', label: 'Username' },
  { value: 'email', label: 'Email' },
  { value: 'comment', label: 'Comment' },
  { value: 'is_active', label: 'Is active' },
  { value: 'is_first_login', label: 'First login' },
  { value: 'system_roles', label: 'System roles' },
  { value: 'org_roles', label: 'Org roles' },
  { value: 'groups', label: 'User groups' },
  { value: 'labels', label: 'Tags' },
];

const ASSET_ATTRIBUTE_OPTIONS: AsyncSelectOption[] = [
  { value: 'name', label: 'Name' },
...
function AttributeNameSelect({ value, options, onChange }: { value: string; options: AsyncSelectOption[]; onChange: (value: string) => void }) {
  const [open, setOpen] = useState(false);
  const selected = options.find((option) => option.value === value);
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button type="button" className="w-full h-10 rounded-md border border-slate-200 bg-white px-3 text-sm text-left flex items-center justify-between hover:bg-slate-50">
          <span className={selected ? 'text-slate-700' : 'text-slate-400'}>{selected?.label || 'Select attribute'}</span>
      ...
      <PopoverContent className="w-[280px] p-1" align="start">
        <div className="max-h-64 overflow-y-auto">
          {options.map((option) => (
            <button key={option.value} type="button" className="w-full rounded px-3 py-2 text-left text-sm hover:bg-slate-50 flex items-center" onClick={() => { onChange(option.value); setOpen(false); }}>
              <CheckCircle2 className={cn('w-4 h-4 mr-2', value === option.value ? 'text-[#009688]' : 'opacity-0')} />
              {option.label}
            </button>
          ))}
        </div>
...
function AttributeEditor({ rules, target, onChange }: { rules: AttributeRule[]; target: 'user' | 'asset'; onChange: (rules: AttributeRule[]) => void }) {
  const updateRule = (index: number, patch: Partial<AttributeRule>) => onChange(rules.map((rule, i) => i === index ? { ...rule, ...patch } : rule));
  const options = target === 'asset' ? ASSET_ATTRIBUTE_OPTIONS : USER_ATTRIBUTE_OPTIONS;
  return (
    <div className="mt-4 space-y-2">
      {rules.map((rule, index) => (
        <div key={index} className="grid grid-cols-[1fr_150px_1fr_36px] gap-2">
          <AttributeNameSelect value={rule.name} options={options} onChange={(name) => updateRule(index, { name })} />
...
export function CommandFilters() {
  ...
  const validate = () => {
    ...
    if ((formData.accountTarget === 'specific' || formData.accountTarget === 'exclude') && formData.accountNames.length === 0) return 'Select at least one account.';
      ...
      <div className="flex flex-col gap-4 bg-white rounded-xl shadow-sm border border-slate-200 min-h-[500px]">
        <div className="flex items-center justify-between p-4 border-b border-slate-100 gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="relative w-64"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><Input placeholder="Search" className="pl-9 h-9 bg-slate-50" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} /></div>
            <Button variant="ghost" size="icon" onClick={loadFilters} disabled={loading}><RotateCw className={cn('w-4 h-4', loading && 'animate-spin')} /></Button>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50/50 text-slate-600 border-b border-slate-100"><tr><th className="px-4 py-3">Name</th><th className="px-4 py-3">Command group</th><th className="px-4 py-3">Priority</th><th className="px-4 py-3">Action</th><th className="px-4 py-3">Active</th><th className="px-4 py-3">Description</th><th className="px-4 py-3 text-right">Actions</th></tr></thead>
            <tbody>
              {loading ? <tr><td colSpan={7} className="py-12 text-center text-slate-500"><Loader2 className="w-5 h-5 animate-spin inline mr-2" />Loading from JumpServer...</td></tr> : filteredData.length === 0 ? <tr><td colSpan={7} className="py-12 text-center text-slate-400">No command filter ACLs found.</td></tr> : filteredData.map((row) => (
                <tr key={row.id} className="border-b border-slate-100 hover:bg-slate-50"><td className="px-4 py-3"><button className="text-[#3498db] hover:underline" onClick={() => openEdit(row.id)}>{row.name}</button></td><td className="px-4 py-3 text-[#3498db]">{row.command_groups_amount ?? 0}</td><td className="px-4 py-3">{row.priority ?? 50}</td><td className="px-4 py-3">{getActionLabel(row.action)}</td><td className="px-4 py-3">{row.is_active !== false ? <span className="flex items-center gap-1 text-[#009688]"><CheckCircle2 className="w-4 h-4" /> Yes</span> : 'No'}</td><td className="px-4 py-3">{row.comment || '-'}</td><td className="px-4 py-3"><div className="flex justify-end gap-1"><Button size="sm" className="h-7 bg-[#009688] hover:bg-[#00796B]" onClick={() => openEdit(row.id)}>Edit</Button><Popover><PopoverTrigger asChild><Button variant="outline" size="sm" className="h-7 w-7 p-0"><MoreHorizontal className="w-4 h-4" /></Button></PopoverTrigger><PopoverContent className="w-32 p-1" align="end"><button onClick={() => handleDelete(row.id)} className="flex w-full items-center gap-2 rounded px-2 py-1.5 text-sm text-red-600 hover:bg-red-50"><Trash2 className="w-4 h-4" />Delete</button></PopoverContent></Popover></div></td></tr>
              ))}
            </tbody>
          </table>
        </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-5xl max-h-[92vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-slate-100"><h2 className="text-xl font-medium text-slate-800">{editingId ? 'Update command filter ACL' : 'Create command filter ACL'}</h2><button onClick={() => setModalOpen(false)} className="p-1 rounded hover:bg-slate-100"><X className="w-5 h-5 text-slate-400" /></button></div>
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
              {error && <div className="rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-600">{error}</div>}
              <section className="space-y-5 pb-8 border-b border-dashed border-slate-200"><h3 className="font-semibold text-slate-800">Basic</h3><div className="grid grid-cols-[140px_1fr] gap-6 items-center"><label className="text-sm text-right"><span className="text-red-500">*</span> Name</label><Input value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} /></div><div className="grid grid-cols-[140px_1fr] gap-6 items-center"><label className="text-sm text-right">Priority</label><Input type="number" min={1} max={100} value={formData.priority} onChange={(e) => setFormData({ ...formData, priority: e.target.value })} /></div></section>
              <section className="pb-8 border-b border-dashed border-slate-200"><h3 className="font-semibold text-slate-800 mb-5">User</h3><div className="grid grid-cols-[140px_1fr] gap-6 items-start"><label className="text-sm text-right mt-1"><span className="text-red-500">*</span> User</label><div><RadioGroup value={formData.userTarget} options={[{ value: 'all', label: 'All users' }, { value: 'specific', label: 'Specific users' }, { value: 'attribute', label: 'Filter by attribute' }]} onChange={(value) => setFormData({ ...formData, userTarget: value })} />{formData.userTarget === 'specific' && <div className="mt-4"><AsyncSelect endpoint="/api/v1/users/users/?fields_size=mini" placeholder="Select users" emptyText="No users found." value={formData.users} onChange={(users) => setFormData({ ...formData, users })} /></div>}{formData.userTarget === 'attribute' && <AttributeEditor target="user" rules={formData.userAttrs} onChange={(userAttrs) => setFormData({ ...formData, userAttrs })} />}</div></div></section>
              <section className="pb-8 border-b border-dashed border-slate-200"><h3 className="font-semibold text-slate-800 mb-5">Asset</h3><div className="grid grid-cols-[140px_1fr] gap-6 items-start"><label className="text-sm text-right mt-1"><span className="text-red-500">*</span> Asset</label><div><RadioGroup value={formData.assetTarget} options={[{ value: 'all', label: 'All assets' }, { value: 'specific', label: 'Specific assets' }, { value: 'attribute', label: 'Filter by attribute' }]} onChange={(value) => setFormData({ ...formData, assetTarget: value })} />{formData.assetTarget === 'specific' && <div className="mt-4"><AsyncSelect endpoint="/api/v1/assets/assets/?fields_size=mini" placeholder="Select assets" emptyText="No assets found." value={formData.assets} onChange={(assets) => setFormData({ ...formData, assets })} /></div>}{formData.assetTarget === 'attribute' && <AttributeEditor target="asset" rules={formData.assetAttrs} onChange={(assetAttrs) => setFormData({ ...formData, assetAttrs })} />}</div></div></section>
              <section className="pb-8 border-b border-dashed border-slate-200"><h3 className="font-semibold text-slate-800 mb-5">Account</h3><div className="grid grid-cols-[140px_1fr] gap-6 items-start"><label className="text-sm text-right mt-1"><span className="text-red-500">*</span> Account</label><div><RadioGroup value={formData.accountTarget} options={[{ value: 'all', label: 'All accounts' }, { value: 'specific', label: 'Specified accounts' }, { value: 'exclude', label: 'Exclude accounts' }, { value: 'none', label: 'None' }]} onChange={(value) => setFormData({ ...formData, accountTarget: value })} />{(formData.accountTarget === 'specific' || formData.accountTarget === 'exclude') && <div className="mt-4"><AsyncSelect method="POST" endpoint="/api/v1/accounts/accounts/username-suggestions/" postBody={{ assets: formData.assets, nodes: [] }} placeholder="Select account" emptyText="No accounts found." value={formData.accountNames} onChange={(accountNames) => setFormData({ ...formData, accountNames })} /></div>}</div></div></section>
              <section className="pb-8 border-b border-dashed border-slate-200"><h3 className="font-semibold text-slate-800 mb-5">Command group</h3><div className="grid grid-cols-[140px_1fr] gap-6 items-start"><label className="text-sm text-right mt-2"><span className="text-red-500">*</span> Command group</label><AsyncSelect endpoint="/api/v1/acls/command-groups/?fields_size=mini" placeholder="Please select command group" emptyText="No command groups found." value={formData.commandGroups} initialOptions={initialCommandGroups} onChange={(commandGroups) => setFormData({ ...formData, commandGroups })} /></div></section>
              <section className="pb-8 border-b border-dashed border-slate-200 space-y-5"><h3 className="font-semibold text-slate-800">Action</h3><div className="grid grid-cols-[140px_1fr] gap-6 items-center"><label className="text-sm text-right">Action</label><div className="relative max-w-[220px]"><select className="w-full h-10 px-3 rounded-md border border-slate-200 bg-white text-sm" value={formData.action} onChange={(e) => setFormData({ ...formData, action: e.target.value as FormState['action'] })}><option value="review">Review</option><option value="accept">Accept</option><option value="reject">Reject</option></select><ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" /></div></div>{formData.action === 'review' && <div className="grid grid-cols-[140px_1fr] gap-6 items-start"><label className="text-sm text-right mt-2"><span className="text-red-500">*</span> Recipients</label><AsyncSelect endpoint="/api/v1/users/users/?fields_size=mini" placeholder="Please select recipients" emptyText="No users found." value={formData.reviewers} initialOptions={initialReviewers} onChange={(reviewers) => setFormData({ ...formData, reviewers })} /></div>}</section>
              <section className="space-y-5"><h3 className="font-semibold text-slate-800">Other</h3><div className="grid grid-cols-[140px_1fr] gap-6 items-center"><label className="text-sm text-right">Active</label><Checkbox checked={formData.active} onCheckedChange={(value) => setFormData({ ...formData, active: !!value })} className="data-[state=checked]:bg-[#009688] data-[state=checked]:border-[#009688]" /></div><div className="grid grid-cols-[140px_1fr] gap-6 items-start"><label className="text-sm text-right mt-2">Description</label><textarea className="w-full h-24 p-3 rounded-md border border-slate-200 text-sm resize-y focus:outline-none focus:ring-2 focus:ring-[#009688]/20" value={formData.comment} onChange={(e) => setFormData({ ...formData, comment: e.target.value })} /></div></section>
            </div>
